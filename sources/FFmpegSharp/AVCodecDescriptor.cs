...
namespace FFmpegSharp
{
    public sealed unsafe class AVCodecDescriptor : IDisposable
    {
        ...
        public AVMediaType Type => (AVMediaType)_handle->type;

        public string Name => new string(_handle->name);

        public string LongName => new string(_handle->long_name);

        public AVCodecProperties Properties => (AVCodecProperties)_handle->props;

        public AVCodecDescriptor(Interop.AVCodecDescriptor* handle)
        {
            if (handle == null)
            {
