...
namespace MediaPlayer
{
    ...
    public class MediaControl : Control
    {
        ...
        private D3DImage _imageSource = null!;
        ...
        private void OnCompositionTargetRendering(object? sender, EventArgs eventArgs)
        {
            ...
            if (_imageSource.IsFrontBufferAvailable && renderingTime > _lastRenderingTime)
            {
                var pSurface = IntPtr.Zero;
                // TODO: Get pointer to Direct3D surface
                if (pSurface != IntPtr.Zero)
                {
                    _imageSource.Lock();
                    _imageSource.SetBackBuffer(D3DResourceType.IDirect3DSurface9, pSurface);
                    // TOD: Render to surface
